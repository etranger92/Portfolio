...
function init() {
  ...
  ticker = setInterval(looper, 1000 / 30);
}
...
function onMouseMove(event) {
  mouseX = -(-(window.innerWidth * 0.3) + event.pageX) * 0.0025;
  mouseY = -(-(window.innerHeight * 0.3) + event.pageY) * 0.01;
  mouseZ = -radius - (Math.abs(-(window.innerHeight * 0.7) + event.pageY) - 200);
}
